...
def show_merchandise(request, index):
    import csv, os
    csv_path = os.path.join(settings.BASE_DIR, 'merchandise.csv')
    with open(csv_path, newline='', encoding='utf-8') as f:
        rows = list(csv.DictReader(f))
    if index < 0 or index >= len(rows):
        raise Http404("Invalid product index")
    data = rows[index]
    return render(request, 'merchandise_detail.html', {'merchandise': data, 'product_index': index})
